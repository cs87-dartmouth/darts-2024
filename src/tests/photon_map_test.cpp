...
{
    // this serves as an example of how to use the KNNSearch and FixedRadiusSearch
    Color3f result;
    float   search_area;
    if (search_count > 0)
    {
    ...
    else
    {
        ...
        FixedRadiusSearch search(pos, search_radius * search_radius, accum_photon);
        photon_map.find(search);
...
void PhotonMapTest::run()
{
    ...
    RNG rng;
    {
        ...
        {
            auto p = generate_photon(rng.rand2f());
