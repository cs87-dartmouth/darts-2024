...
protected:
    ...
    float                                    m_density{1.0f};       ///< density multiplier
...
NanoVDBMedium::NanoVDBMedium(const json &j) : Medium(j)
{
    ...
    // nanovdb::BBoxR bbox = m_density_grid->worldBBox();
    // m_bbox.enclose({bbox.min()[0], bbox.min()[1], bbox.min()[2]});
    // m_bbox.enclose({bbox.max()[0], bbox.max()[1], bbox.max()[2]});
    ...
    m_total = Vec3f(m_density * max * (m_sigma_s + m_sigma_a));
...
{
    auto  p       = m_xform.inverse().point(p_);
    auto  p_index = m_density_grid->worldToIndexF(nanovdb::Vec3f(p.x, p.y, p.z));
    float d       = m_density * Lookup(m_density_grid->tree())(p_index);

    return std::make_tuple(d * m_sigma_a, d * m_sigma_s, m_total - (m_sigma_a + m_sigma_s) * d);
}

bool NanoVDBMedium::sample_free_flight(const Ray3f &ray, int channel, Sampler &sampler, HitRecord &hit, Color3f &f,
                                       Color3f &p) const
{
